...
public final class KeyGenerator {
    ...
    if (ZERO.equals(fn.remainder(e))) {
      ...
      if (ZERO.equals(fn.remainder(e))) {
        ...
        if (ZERO.equals(fn.remainder(e))) {
          ...
          if (ZERO.equals(fn.remainder(e))) {
            LOGGER.severe("Create a RSA parameter failure, re execution");// 如果模n与上面的素数都不互素的话，创建RSA参数失败
  ...
  private static class Euclid {
    ...
    private BigInteger d;// x，y成员变量，用于保存generateD()计算时的中间值
    ...
    private void generateD() {
      ...
      LOGGER.finest("gcd = " + gcd);// 先打印查看最大公约数是否为1，保证无异常
      LOGGER.finest("x = " + x);
      LOGGER.finest("y = " + y);
      /*
      ...
      d = y;
      LOGGER.finest("d = " + d);
      LOGGER.finest("e * d (mod φ(n)) = " + (e.multiply(d).remainder(fn)));
    }
